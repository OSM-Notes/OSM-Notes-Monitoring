...
SELECT
    ...
    PERCENTILE_CONT(
        0.95
    ) WITHIN GROUP (ORDER BY metric_value::numeric) AS p95_response_time_ms,
...
SELECT
    AVG(
        CASE
            WHEN
                metric_name = 'cache_hit_rate_percent' THEN metric_value::numeric
        END
    ...
    SUM(
        CASE
            WHEN metric_name = 'cache_hits' THEN metric_value::numeric ELSE 0
        END
    ) AS cache_hits,
    SUM(
        CASE
            WHEN metric_name = 'cache_misses' THEN metric_value::numeric ELSE 0
        END
    ) AS cache_misses
FROM metrics
WHERE component = 'wms'
      AND metric_name IN (
      'cache_hit_rate_percent', 'request_count', 'cache_hits', 'cache_misses'
      )
      AND timestamp > NOW() - interval '1 hour';
...
SELECT
    current_period.avg_response_time AS current_avg_ms,
    previous_period.avg_response_time AS previous_avg_ms,
    CASE
        WHEN previous_period.avg_response_time > 0 THEN
            (
                (
                    current_period.avg_response_time - previous_period.avg_response_time
                ) / previous_period.avg_response_time * 100
            )
        ELSE 0
