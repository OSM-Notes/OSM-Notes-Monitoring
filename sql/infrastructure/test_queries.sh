#!/usr/bin/env bash
#
# Test SQL Queries for Infrastructure Monitoring
...
validate_sql_syntax() {
    ...
    # Basic keyword validation (very basic, not a full SQL parser)
    # The database connection check will catch real syntax errors.
    return 0
}
...
extract_query() {
    ...
    # Try to extract query by number (if queries are numbered)
    awk -v query_num="${query_num}" '
        /^-- Query [0-9]+:/ {
            current_query = $3
            gsub(/:/, "", current_query)
            query_start = NR
...
test_sql_file() {
    ...
    local query_count
    query_count=$(grep -cE "^-- Query [0-9]+:" "${sql_file}" 2>/dev/null || echo "0")
    # Remove any non-numeric characters (like newlines)
    query_count="${query_count//[^0-9]/}"
    ...
    for ((i=1; i<=query_count; i++)); do
        ...
        local query
        query=$(extract_query "${sql_file}" "${i}")
        
        if [[ -z "${query}" ]]; then
            print_message "${YELLOW}" "    Query ${i}: ⚠ Could not extract query"
            continue
        fi
...
test_all_queries() {
    ...
    local sql_files=(
        "${SCRIPT_DIR}/connectivity.sql"
        "${SCRIPT_DIR}/resources.sql"
    )
...
main() {
    ...
    print_message "${GREEN}" "SQL Queries Test Suite - Infrastructure Monitoring"
