...
SELECT
    error_events.execution_time AS error_time,
    recovery_events.execution_time AS recovery_time,
    EXTRACT(
        EPOCH FROM (
            recovery_events.execution_time - error_events.execution_time
        )
    ) AS recovery_seconds
FROM error_events
    INNER JOIN
        recovery_events ON recovery_events.recovery_seq = error_events.error_seq
ORDER BY error_events.execution_time DESC
LIMIT 10;
...
SELECT
    ...
    ROUND(
        (
            error_count - LAG(error_count) OVER (ORDER BY day)
        ) * 100.0 / NULLIF(LAG(error_count) OVER (ORDER BY day), 0),
        2
    ) AS percent_change
FROM daily_errors
ORDER BY day DESC;
