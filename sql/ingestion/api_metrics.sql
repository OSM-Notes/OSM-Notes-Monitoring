...
SELECT
    ...
    AVG(
        CASE
            WHEN
                metric_name = 'api_success_rate_percent' THEN metric_value::numeric
        END
    ) AS avg_success_rate_percent,
    COUNT(
        CASE WHEN metric_name = 'api_success_rate_percent' THEN 1 END
    ) AS data_points
FROM metrics
WHERE component = 'ingestion'
...
SELECT
    ...
    AVG(
        CASE
            WHEN metric_name = 'api_response_time_ms' THEN metric_value::numeric
        END
...
SELECT
    ...
    SUM(
        CASE
            WHEN
                metric_name = 'api_errors_4xx_count' THEN metric_value::numeric
            ELSE 0
        END
    ) AS errors_4xx_count,
    SUM(
        CASE
            WHEN
                metric_name = 'api_errors_5xx_count' THEN metric_value::numeric
            ELSE 0
        END
    ) AS errors_5xx_count
FROM metrics
...
SELECT
    ...
    AVG(
        CASE
            WHEN
                metric_name = 'api_requests_per_minute' THEN metric_value::numeric
        END
    ) AS avg_requests_per_minute,
    AVG(
        CASE
            WHEN
                metric_name = 'api_requests_per_hour' THEN metric_value::numeric
        END
...
SELECT
    ...
    SUM(
        CASE
            WHEN
                metric_name = 'api_rate_limit_hits_count' THEN metric_value::numeric
            ELSE 0
        END
    ) AS rate_limit_hits
FROM metrics
...
SELECT
    ...
    AVG(
        CASE
            WHEN
                metric_name = 'api_response_size_bytes' THEN metric_value::numeric
        END
    ) AS avg_response_size_bytes,
    AVG(
        CASE
            WHEN
                metric_name = 'api_notes_per_request' THEN metric_value::numeric
        END
    ) AS avg_notes_per_request
FROM metrics
WHERE component = 'ingestion'
