...
SELECT
    ...
    COUNT(
        *
    ) FILTER (WHERE latitude IS NULL OR longitude IS NULL) AS null_coordinates,
...
SELECT
    ...
    ) * 100.0 / NULLIF(COUNT(*), 0), 2) AS completeness_percent
FROM notes;
...
FROM note_comments
    LEFT JOIN notes ON note_comments.note_id = notes.id
...
SELECT
    ...
    COUNT(*) AS count
FROM notes
    LEFT JOIN note_comments ON notes.id = note_comments.note_id
WHERE note_comments.id IS NULL
UNION ALL
SELECT
...
FROM note_comments
    LEFT JOIN
        note_comment_texts ON note_comments.id = note_comment_texts.comment_id
WHERE note_comment_texts.id IS NULL;
...
SELECT
    ...
    COUNT(
        *
    ) FILTER (WHERE longitude < -180 OR longitude > 180) AS invalid_longitude,
    COUNT(
        *
    ) FILTER (WHERE latitude IS NULL OR longitude IS NULL) AS null_coordinates,
...
SELECT
    ...
    ROUND(
        valid_notes * 100.0 / NULLIF(total_notes, 0), 2
