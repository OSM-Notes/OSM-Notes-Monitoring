...
SELECT
    ...
    EXTRACT(
        EPOCH FROM (NOW() - MAX(execution_time))
    ) AS seconds_since_last_execution
FROM processing_log
...
SELECT
    ...
    ROUND(
        COUNT(
            *
        ) FILTER (WHERE status = 'success') * 100.0 / NULLIF(COUNT(*), 0),
        2
...
SELECT
    expected_executions.expected_time AS missing_execution_time,
    EXTRACT(
        EPOCH FROM (NOW() - expected_executions.expected_time)
    ) / 3600 AS hours_ago
FROM expected_executions
    LEFT JOIN
        actual_executions ON
            DATE_TRUNC(
                'hour', expected_executions.expected_time
            ) = actual_executions.execution_time
WHERE actual_executions.execution_time IS NULL
ORDER BY expected_executions.expected_time DESC;
